...
When you choose a photo for a live scan or a model-label refinement and tap **I understand**, the app sends the image to ClutterCash's Cloudflare Worker and then to the OpenAI API for analysis.

- The Worker accepts JPEG, PNG, and WebP images up to 8 MB.
- The Worker processes image bytes in memory and is designed not to write the image to disk. Worker responses use `Cache-Control: no-store`, and the OpenAI Responses request uses `store: false`.
- OpenAI is the AI processor for this beta. OpenAI states API data is not used to train or improve its models by default unless the account explicitly opts in. Default abuse-monitoring logs may retain customer content for up to 30 days, with limited exceptions described in OpenAI's data-controls documentation. Review OpenAI's applicable terms and privacy information before using live analysis.
- Crop or cover unique serial numbers before uploading a label photo, leaving useful maker/model details visible. We ask the model to omit serials and omit dedicated serial fields from the response. Limited filtering removes exact serial text only if the provider explicitly identifies it in an unwanted serial field; unmarked or changed serial text can remain. Marketplace queries use the returned product identity instead of additional label-derived search text. We cannot guarantee exclusion from results, local saved data or marketplace searches you open. Review and correct private details before saving or researching. Cropping must happen before upload: response filtering does not remove serials from the photo sent to OpenAI.
...
- **Uploaded images:** ClutterCash does not provide a cloud gallery or account-based image deletion tool because it does not intentionally store your uploaded image on its Worker. OpenAI's handling is governed by OpenAI's policies; ClutterCash cannot delete provider abuse-monitoring data through the app.
...
- **Anonymous network controls:** For anonymous analyses, the Worker hashes the Cloudflare-supplied connection address together with the UTC date and stores only that day-scoped digest and accepted-attempt count. It does not store or log the raw address in this analysis limiter. This modest six-attempt daily address limit is shared by users behind the same address; a small shared trial pool also applies. Hashing is not anonymization. Old day records no longer affect quotas but remain until operator cleanup; no automatic deletion is implemented. Clearing local projects does not erase them. Cloudflare/network providers may separately retain connection data under their own policies.
...
- **Revoked credentials:** A revoked device/invite hash is retained as an indefinite denial marker to prevent that same credential being restored by an old approval. Revocation does not delete prior quota counters or cancel already-authorized requests.
- **Diagnostic rate limits:** One aggregate minute/count record bounds anonymous and total accepted telemetry, independently of AI allowances. It contains no user/token/network identifiers, is overwritten on the next admitted event in a new minute, and remains while idle; no history of minute counters accumulates.
