...
test('public paid scan and status use verified sessions with atomic SQLite quota; no fallback or redirect grant',async()=>{
 ...
 const handler=createHandler({now:()=>${timestamp},alertSender:async()=>true,providerMetricSender:async()=>{},fetcher:async()=>Response.json({id:'resp_fixture',status:'completed',output:[{type:'message',content:[{type:'output_text',text:JSON.stringify({sceneSummary:'Shelf',items:[],exactName:'Camera',confidence:'high'})}]}],usage:{input_tokens:1,output_tokens:1,total_tokens:2}})});
